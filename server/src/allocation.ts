...
export function allocateInventory(
  ...
  // Keep an original inventory snapshot for theoretical max buildable calculations
  ...
  // Calculate how much each product CAN build with remaining inventory
  ...
  for (const product of sortedAllProducts) {
    ...
    // Allocate this amount to the product
    if (maxCanBuild > 0) {
 ...
 * based on current available inventory (independent of allocation decisions).
 */
...
export function allocateByDemandRatio(
  ...
  // Sort products with targets by priority (lower number = higher priority)
  const sortedProductsWithTargets = productsWithTargets.sort((a, b) => {
    return a.priority - b.priority;
  });

  // Phase 1: Fair allocation to products with targets
  // Use round-robin: each product gets 1 unit per cycle (if it needs more and can build)
  // This ensures products with shared bottleneck parts get equal fairness
  let madeProgress = true;
  while (madeProgress) {
    madeProgress = false;
    
    // Try each product in priority order, but only ONE unit per product per cycle
    for (const product of sortedProductsWithTargets) {
      const target = targets[product.name];
      const currentCompleted = completed.get(product.id) || 0;
      
      // Only allocate 1 unit per round if haven't reached target and can build
      if (currentCompleted < target.minStock && canBuildOne(product, availableInventory)) {
        deductPartsForOne(product, availableInventory);
        completed.set(product.id, currentCompleted + 1);
  ...
  // Phase 2: Allocate remaining parts to ALL products with targets in round-robin fashion
  // This allows products to build beyond their minStock targets
  madeProgress = true;
  while (madeProgress) {
    madeProgress = false;
    
    for (const product of sortedProductsWithTargets) {
      if (canBuildOne(product, availableInventory)) {
        deductPartsForOne(product, availableInventory);
        completed.set(product.id, (completed.get(product.id) || 0) + 1);
        ...
        madeProgress = true;
        // After each successful build, break to go to next round for fair distribution
        break;
      }
    }
  }

  // Phase 3: Products without targets get NO allocation - they remain theoretical only
  // Don't allocate any parts to products with demand = 0
